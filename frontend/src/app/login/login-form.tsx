...
export function LoginForm() {
  ...
  function redirectAfterLogin(painelUrl?: string) {
    return redirect || painelUrl || "/conta";
  }

  async function syncAndRedirect() {
    const res = await fetch("/api/auth/sync", { method: "POST" });
    const data = await res.json().catch(() => ({}));
    const dest = redirectAfterLogin(data.painel_url);
    router.push(dest);
    router.refresh();
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (mode === "signup") {
      const { data, error } = await supabase.auth.signUp({
        ...
        password,
        options: {
          data: { nome, tipo_conta: tipoConta },
          emailRedirectTo: `${callbackUrl()}?next=${encodeURIComponent(redirectAfterLogin())}`,
        },
      });
      if (error) {
        setMessage(mapAuthError(error.message));
    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-50 px-4 py-10">
      <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
        ...
        <p className="mt-6 text-center text-xs text-slate-400">
          Niveis: <strong>usuario</strong>, <strong>empresa</strong> e <strong>admin</strong> (promovido pela equipe).
        </p>
      </div>
    </div>
  );
